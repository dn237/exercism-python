...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    if temperature < 800 and neutrons_emitted > 500 and product < 5e5:
        return True
    else:
        return False

#print(is_criticality_balanced(750, 500))
...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    precentage_efficiency = (generated_power/theoretical_max_power)*100

    if precentage_efficiency >= 80:
        return 'green'
    elif 60 <= precentage_efficiency < 80:
        return 'orange'
    elif 30 <= precentage_efficiency < 60:
        return 'red'
    elif precentage_efficiency < 30:
        return 'black'
    return
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    """
    temp_rn = temperature * neutrons_produced_per_second
    low_threshold = threshold * 0.9
    high_threshold = threshold * 1.1


    if temp_rn < low_threshold:
        return 'LOW'
    elif low_threshold <= temp_rn <= high_threshold:
        return 'NORMAL'
    else:
        return 'DANGER'
    return

#print(fail_safe(temperature=5, neutrons_produced_per_second=1, threshold=5000))